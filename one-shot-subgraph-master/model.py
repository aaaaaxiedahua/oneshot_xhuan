...
class GNNLayer(torch.nn.Module):
    ...
    def forward(self, q_sub, q_rel, r_idx, hidden, edges, n_node, shortcut=False,
                edge_weights=None, virtual_rel_embeds=None, n_virtual=0):
        sub = edges[:,0]
        rel = edges[:,1]
        obj = edges[:,2]
        hs = hidden[sub]
...
class GNN_auto(torch.nn.Module):
    def __init__(self, params, loader):
        ...
        for i in range(self.n_layer):
            self.gnn_layers.append(GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, self.n_rel, act=act, n_extra_rel=n_extra_rel))
    ...
    def forward(self, q_sub, q_rel, subgraph_data, mode='train'):
        ...
        for i in range(self.n_layer):
            ...
            hidden = self.gnn_layers[i](q_sub, q_rel, curr_batch, hidden, curr_edges, n_node,
                                        shortcut=self.params.shortcut, edge_weights=curr_weights,
                                        virtual_rel_embeds=curr_vr, n_virtual=curr_nv)
